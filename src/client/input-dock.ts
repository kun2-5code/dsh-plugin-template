/**
 * 输入区 Dock（conversation.input.dock 插槽）：在输入卡片上方渲染一整行
 * 状态条。该插槽是 session 级 list——注册时通过 options.inject 工厂收到
 * sessionId，组件经 props 拿到注入的面；owner 只传点状快照（session/input），
 * 需要实时数据时应走框架标准 hook（useSession 等），不要在组件里订阅。
