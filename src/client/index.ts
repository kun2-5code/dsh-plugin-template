 ...
 * input-dock），各自的注册函数在 apply 里按序调用。
 *
 * 关于"开箱即用"：本入口注册的插槽全部是纯声明式 UI 注册，不受 harness 的
 * WEB_SETTINGS_NAMESPACES 白名单影响——侧栏按钮、输入区 Dock 装完即用；
 * 只有配置卡片的数据路径（settings 命名空间）受白名单门控，卡片本身也会
 * 渲染说明状态而不是消失。详见 docs/ui-surfaces.md。
