...
/**
 * 与内置 PluginCard 相同的展开箭头。内置卡片用 ui-primitives 的
 * IconChevronDownOutline14；这里内联同一个 14×14 SVG（viewBox 与 path 逐字相同，
 * fill 走 currentColor，配合 dtpl-chevron 样式），视觉上与内置卡片完全一致。
 * 不 import ui-primitives：浏览器侧平台包未纳入模板依赖集（且依赖整棵
 * markdown 渲染树），声明为依赖会让模板的 `pnpm install --frozen-lockfile`
 * （CI）解析失败；内联同款字形零依赖、零漂移风险。
 */
